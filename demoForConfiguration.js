...
YFI.addPlug("business", "../common/plug_business")
...
module.exports = {
    /****** 登出token清除 ******/
    "removeToken": {
        filters: [YFI.plug.commonFilter.authenticationManage, "{{#token}}"],
        routerOperate: [
            { key: "清除缓存", fun: YFI.plug.cache.remove, args: ["token:" + "{{#token}}"] }
        ],
    ...
    "login": {
        ...
        routerOperate: [
            ...
            { key: "缓存凭据", fun: YFI.plug.cache.set, args: ["token:" + "{{~lastResult}}", 3600, "{{#name}}"] },
            { key: "获取用户数据", fun: YFI.plug.mysql.seleteSingle, args: ["account", "type", [{ field: "name", value: "{{#name}}" }]] },
            { key: "返回登录凭据", fun: YFI.plug.http.responseEnd, args: [{ token: "{{~results.生成登录凭据}}", type: "{{~results.获取用户数据}}" }] }
        ],
    ...
    //查询用户列表
    "getUserList": {
        filters: [YFI.plug.commonFilter.authenticationManage],
        routerOperate: [
        ...
        export: {
            ...
            beforeExport: { fun: YFI.plug.business.verifyExportPwd, args: ["{{#pwd}}"] },
    ...
    "getLogList": {
        ...
        routerOperate: [
            ...
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取日志集合", fun: YFI.plug.mysql.getPageDataBySelect, args: ["log", "*", [{ field: "main_type", value: "{{#main_type}}", compareSymbol: "=" }, { field: "child_type", value: "{{#child_type}}", compareSymbol: "=" }, { field: "create_time", value: "{{#create_time}}", compareSymbol: "between" }, { field: "ip", value: "{{#ip}}", compareSymbol: "like" }, { field: "login_name", value: "{{#login_name}}", compareSymbol: "like" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}}", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "返回日志集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ]
    },

    //新增/编辑用户
    "addUser": {
        filters: [YFI.plug.commonFilter.authenticationManage, "{{#name}}"]
        ,
        routerOperate: [
            { key: "获取当前时间", fun: YFI.plug.date.getFormat, args: ["YYYY-MM-DD HH:mm:ss"] },
            ...
            { key: "获取密码加密", fun: YFI.plug.encrypt.encode, args: ["{{#password}}", CONFIG.server_salt] },
            { key: "添加用户", fun: YFI.plug.mysql.insert, args: ["account", { id: "{{#id}}", name: "{{#name}}", password: "{{~lastResult}}", type: "{{#type}}", create_time: "{{~results.获取当前时间}}" }, { keyName: "id", isEmptyNotUpdateed: "password", emptyValue: "79c515fffba583982cdba9c4033fb544" }] }
        ]
        // YFI.plug.vm2.convertToVM2Code("moment(new Date()).format('YYYY-MM-DD');", {}, ["moment"])
    },
    ...
    "delUser": {
        ...
        ,
        routerOperate: [
            { key: "删除用户", fun: YFI.plug.mysql.delete, args: ["account", { id: "{{#id}}" }] }
        ]
    },
    ...
    "getMainUsers": {
        ...
        routerOperate: [
            ...
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取主账号集合", fun: YFI.plug.mysql.getPageDataBySelect, args: [[{ table: "user", alias: "u" }, { table: "device", alias: "d", join: "left join", equal: [{ left: "d.tel", right: "u.tel" }] }], "u.tel,u.status,d.device_id,u.create_time", [{ field: "u.tel", value: "{{#tel}}", compareSymbol: "like" }, { field: "d.device_id", value: "{{#device_id}}", compareSymbol: "like" }, { field: "u.status", value: "{{#status}}" }, { field: "u.create_time", value: "{{#create_time}}", compareSymbol: "between" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}},u.tel", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "手机号脱敏处理", fun: YFI.plug.business.formatListPhoneNODesensitization, args: ["{{~results.lastResult}}", "tel"] },
            { key: "返回主账号集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ],
        export: {
            fileName: "主账号清单",
            fields: [
    ...
    "getChildrenUsers": {
        ...
        routerOperate: [
            ...
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取子账号集合", fun: YFI.plug.mysql.getPageDataBySelect, args: ["user", "tel,parent,create_time", [{ field: "tel", value: "{{#tel}}", compareSymbol: "like" }, { field: "parent", value: "{{#parent}}", compareSymbol: "like" }, { field: "parent", value: "", compareSymbol: "!=" }, { field: "create_time", value: "{{#create_time}}", compareSymbol: "between" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}}", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "手机号脱敏处理", fun: YFI.plug.business.formatListPhoneNODesensitization, args: ["{{~results.lastResult}}", ["tel", "parent"]] },
            { key: "返回子账号集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ],
        export: {
            fileName: "子账号清单",
            fields: [
    ...
    "getSIMCharge": {
        filters: [YFI.plug.commonFilter.authenticationManage],
        routerOperate: [
            { key: "获取排序字段", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#orderProp}}", right: undefined, result: "o.create_time", then: "{{#orderProp}}" }] },//create_time
            { key: "获取正逆排序关键字", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#orderAsc}}", right: true, result: "ASC", then: "DESC" }] },//DESC
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取充值记录", fun: YFI.plug.mysql.getPageDataBySelect, args: [[{ table: "orders", alias: "o" }, { table: "device", alias: "d", join: "join", equal: [{ left: "o.order_owner", right: "d.device_id" }] }], "d.tel,d.device_id,d.device_tel,d.iccid,o.goods_type,o.pay_type,o.status,o.create_time", [{ field: "d.tel", value: "{{#tel}}", compareSymbol: "like" }, { field: "d.device_id", value: "{{#device_id}}", compareSymbol: "like" }, { field: "d.device_tel", value: "{{#device_tel}}", compareSymbol: "like" }, { field: "d.iccid", value: "{{#iccid}}", compareSymbol: "like" }, { field: "o.goods_type", value: "{{#goods_type}}" }, { field: "o.pay_type", value: "{{#pay_type}}" }, { field: "o.status", value: "{{#status}}" }, { field: "o.create_time", value: "{{#create_time}}", compareSymbol: "between" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}},o.ID", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "手机号脱敏处理", fun: YFI.plug.business.formatListPhoneNODesensitization, args: ["{{~results.lastResult}}", "tel"] },
            { key: "返回充值记录集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ],
        export: {
            fileName: "SIM卡充值清单",
            fields: [
    ...
    "getDeviceStatus": {
        ...
        routerOperate: [
            ...
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取设备状态", fun: YFI.plug.mysql.getPageDataBySelect, args: [[{ table: "device", alias: "d" }, { table: "user", alias: "u", join: "left join", equal: [{ left: "d.tel", right: "u.tel" }] }], "d.device_id,d.tel,d.device_tel,case when d.bind_time is null then 0 else 1 end iBind,d.status,d.bind_time,d.create_time", [{ field: "d.device_id", value: "{{#device_id}}", compareSymbol: "like" }, { field: "d.device_tel", value: "{{#device_tel}}" }, { field: "d.bind_time", value: "{{#iBind}}", compareSymbol: "judge null" }, { field: "d.tel", value: "{{#tel}}", compareSymbol: "like" }, { field: "d.bind_time", value: "{{#bind_time}}", compareSymbol: "between" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}},d.device_id", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "手机号脱敏处理", fun: YFI.plug.business.formatListPhoneNODesensitization, args: ["{{~results.lastResult}}", "tel"] },
            { key: "返回设备状态集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ],
        export: {
            fileName: "设备状态清单",
            fields: [
    ...
    "getDeviceEvent": {
        filters: [YFI.plug.commonFilter.authenticationManage],
        routerOperate: [
            { key: "获取排序字段", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#orderProp}}", right: undefined, result: "de.create_time", then: "{{#orderProp}}" }] },//create_time
            { key: "获取正逆排序关键字", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#orderAsc}}", right: true, result: "ASC", then: "DESC" }] },//DESC
            { key: "获取页码", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#current}}", right: undefined, result: undefined, then: "{{#current}}" }] },//DESC
            { key: "获取单页总数", fun: YFI.plug.expression.ask, args: [{ symbol: "===", left: "{{#size}}", right: undefined, result: undefined, then: "{{#size}}" }] },//DESC
            { key: "获取设备事件记录", fun: YFI.plug.mysql.getPageDataBySelect, args: [[{ table: "device_event", alias: "de" }, { table: "device", alias: "d", join: "join", equal: [{ left: "de.device_id", right: "d.device_id" }] }], "d.device_id,de.event_type,d.tel,de.create_time", [{ field: "d.device_id", value: "{{#device_id}}", compareSymbol: "like" }, { field: "de.event_type", value: "{{#event_type}}" }, { field: "d.tel", value: "{{#tel}}", compareSymbol: "like" }, { field: "de.create_time", value: "{{#create_time}}", compareSymbol: "between" }], "{{~results.获取排序字段}} {{~results.获取正逆排序关键字}},d.device_id", "{{~results.获取页码}}", "{{~results.获取单页总数}}"] },
            { key: "手机号脱敏处理", fun: YFI.plug.business.formatListPhoneNODesensitization, args: ["{{~results.lastResult}}", "tel"] },
            { key: "返回设备事件记录集合", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ],
        export: {
            fileName: "设备状态清单",
            fields: [
    ...
    "getAppConfig": {
        filters: [YFI.plug.commonFilter.authenticationManage],
        routerOperate: [
            { key: "获取用户数据", fun: YFI.plug.mysql.select, args: ["goods", "*"] },
            { key: "返回数据", fun: YFI.plug.http.responseEnd, args: ["{{~lastResult}}"] }
        ]
    },
    "saveAppConfig": {
        filters: [YFI.plug.commonFilter.authenticationManage, "{{#yearCost}}", "{{#monthCost}}"]//"{{#activeCost}}", 
        ,
        routerOperate: [
